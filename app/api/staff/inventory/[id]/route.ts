...
// PATCH /api/staff/inventory/[id] - แก้ไขข้อมูลอะไหล่
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const part = await prisma.$transaction(async (tx) => {
      ...
      const hasStockChange = stock_qty !== undefined && stock_qty !== current.stock_qty;
...
// DELETE /api/staff/inventory/[id] - ลบอะไหล่
